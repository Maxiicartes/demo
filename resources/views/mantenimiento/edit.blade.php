        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-xl sm:rounded-lg">
                <div class="p-6 sm:px-20 bg-white border-b border-gray-200">
                    ...
                    <form action="{{ route('mantenimiento.update', $mantenimiento->id) }}" method="POST" enctype="multipart/form-data">
                        @csrf
                        @method('PUT')

                        <div class="mt-4">
                            <label for="nombre_producto" class="block font-medium text-sm text-gray-700">Nombre del Producto</label>
                            <input type="text" name="nombre_producto" id="nombre_producto" class="form-control" value="{{ $mantenimiento->nombre_producto }}" />
                        </div>
                        ...
                        <div class="mt-4">
                            ...
                            @if ($mantenimiento->foto_antes)
                                <img src="{{ asset('storage/' . $mantenimiento->foto_antes) }}" alt="Foto Antes" class="img-thumbnail mt-2" style="max-width: 200px;">
                            @endif
                        </div>
                        ...
                        <div class="mt-4">
                            ...
                            @if ($mantenimiento->foto_despues)
                                <img src="{{ asset('storage/' . $mantenimiento->foto_despues) }}" alt="Foto Después" class="img-thumbnail mt-2" style="max-width: 200px;">
                            @endif
                        </div>

                        <div class="mt-4">
                            <label for="motivo" class="block font-medium text-sm text-gray-700">Motivo</label>
                            <textarea name="motivo" id="motivo" class="form-control">{{ $mantenimiento->motivo }}</textarea>
                        </div>

                        <div class="mt-4">
                            <label for="fecha_ultima_actualizacion" class="block font-medium text-sm text-gray-700">Fecha de Última Actualización</label>
                            <input type="date" name="fecha_ultima_actualizacion" id="fecha_ultima_actualizacion" class="form-control" value="{{ $mantenimiento->fecha_ultima_actualizacion }}" />
                        </div>

                        <div class="mt-4">
                            <label for="fecha_mantenimiento" class="block font-medium text-sm text-gray-700">Fecha de Mantenimiento</label>
                            <input type="date" name="fecha_mantenimiento" id="fecha_mantenimiento" class="form-control" value="{{ $mantenimiento->fecha_mantenimiento }}" />
                        </div>

                        <div class="mt-4">
                            <label for="responsable" class="block font-medium text-sm text-gray-700">Responsable</label>
                            <input type="text" name="responsable" id="responsable" class="form-control" value="{{ $mantenimiento->responsable }}" />
                        </div>

                        <div class="mt-4">
                            <label for="unidad" class="block font-medium text-sm text-gray-700">Unidad</label>
                            <input type="text" name="unidad" id="unidad" class="form-control" value="{{ $mantenimiento->unidad }}" />
                        </div>

                        <div class="mt-4">
                            <label for="tecnico_nombre" class="block font-medium text-sm text-gray-700">Nombre del Técnico</label>
                            <input type="text" name="tecnico_nombre" id="tecnico_nombre" class="form-control" value="{{ $mantenimiento->tecnico_nombre }}" />
                        </div>

                        <div class="mt-4">
                            <label for="tecnico_rut" class="block font-medium text-sm text-gray-700">RUT del Técnico</label>
                            <input type="text" name="tecnico_rut" id="tecnico_rut" class="form-control" value="{{ $mantenimiento->tecnico_rut }}" />
                        </div>

                        <div class="mt-4">
                            <label for="empresa" class="block font-medium text-sm text-gray-700">Empresa</label>
                            <input type="text" name="empresa" id="empresa" class="form-control" value="{{ $mantenimiento->empresa }}" />
                        </div>

                        <div class="mt-4">
                            <label for="tipo_visita" class="block font-medium text-sm text-gray-700">Tipo de Visita</label>
                            <input type="text" name="tipo_visita" id="tipo_visita" class="form-control" value="{{ $mantenimiento->tipo_visita }}" />
                        </div>

                        <div class="form-group">
                            <strong>Código QR:</strong>
                            {!!QrCode::size(300)->generate('http://127.0.0.1:8000/revisar-mantenimiento/'.$mantenimiento->id) !!}
                        </div>
